...
class UpdateUser(views.APIView):
    ...
    def post(self, request):
        try:
            ...
            user_obj.last_name = data['lasttname']
...
class RegisterUserView(views.APIView):
    def post(self, request):
        ...
        if serializers.is_valid():
            serializers.save()
            return Response({'error': False, 'message': f"A user with crated for username {username}"})
        return Response({'error': True, 'message': "A user with that username already exists."})
